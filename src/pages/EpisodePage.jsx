    ...
    <div className="epis-page">
      <div className="epis-page__header">
        <div className="epis-page__header-img">
          {episode?.series ? (
            <img src={episode?.imageName} alt={episode?.series} />
          ) : null}
        </div>
...
const View = ({ episode }) => {
  const { premiere, series, name, plot = 'нет данных', characters } = episode
    ...
    <div className="epis-page__content">
      <div className="container">
        <div className="epis-page__caption">
          ...
          <p className="epis-page__subtitle">серия {series}</p>
        </div>
        <p className="epis-page__descr">{plot}</p>
